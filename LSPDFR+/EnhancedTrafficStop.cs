...
namespace LSPDFR_
{
    internal class EnhancedTrafficStop
    {
        ...
        public void IssueTicket(bool SeizeVehicle)
        {
            GameFiber.StartNew(delegate
            {
                // ✅ Start normal ticket issuing immediately
                Game.LocalPlayer.Character.Tasks.AchieveHeading(Game.LocalPlayer.Character.CalculateHeadingTowardsEntity(Suspect));
                ...
                // 🛡️ Court Summons Background Paperwork Logic with Bonus Message
                if (Menus.IsCourtSummonsSelected())
                {
                    Persona suspectPersona = Functions.GetPersonaForPed(Suspect);
                    if (suspectPersona != null)
                    {
                        GameFiber.StartNew(delegate
                        {
                            Game.DisplayNotification("~y~Court processing initiated. Court paperwork being filed...");

                            Random rnd = new Random();
                            int delayMilliseconds = rnd.Next(60000, 420001); // 1 to 7 minutes

                            GameFiber.Wait(delayMilliseconds);

                            CourtSystem.CreateCourtCase(suspectPersona, SuspectVehicle, SelectedOffences);
                            Game.DisplayNotification("~g~Court Case created and saved for Court Summons.");
                        });
                    }
                }
            });
