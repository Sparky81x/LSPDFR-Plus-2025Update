    ...
    internal static class LSPDFRPlusHandler
    {
        ...
        public static void BetaCheck()
        {
            GameFiber.StartNew(delegate
            {
                ...
                Game.LogTrivial("LSPDFR+, by Albo1125 updated by Sparky for LSPDFR 0.4.9");
                GameFiber.Wait(6000);
                Game.DisplayNotification("~b~LSPDFR+~s~, by ~b~Albo1125, ~s~updated by ~b~Sparky for ~b~LSPDFR 0.4.9 ~g~loaded successfully.");
